...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # debug
        # from latte.data.utils.visualize import draw_points_image, draw_bird_eye_view
...
def pose_generation(root_dir, pose_out_dir):
    from kiss_icp.datasets import dataset_factory
    from kiss_icp.pipeline import OdometryPipeline
    from tqdm import tqdm
    
    seq_list = os.listdir(osp.join(root_dir, "sequences"))
    for seq_ID in tqdm(seq_list):
        seq_out_dir = osp.join(pose_out_dir, "{}".format(seq_ID))
...
if __name__ == '__main__':
    root_dir = '/data1/semantic_kitti/preprocess'
    out_dir = '/data1/semantic_kitti/preprocess'
    pose_root_dir = 'latte/datasets/semantic_kitti/dataset'
    pose_out_dir = 'latte/datasets/semantic_kitti/poses'
    # preprocess('val', root_dir, out_dir)
    # preprocess('train', root_dir, out_dir)
    # preprocess('test', root_dir, out_dir)
    pose_generation(pose_root_dir, pose_out_dir)
