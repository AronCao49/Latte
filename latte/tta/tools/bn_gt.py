...
def bn_gt(
    ...
    for iteration, data_batch_trg in enumerate(train_dataloader_trg):
        ...
        with torch.no_grad():
            setup_validate()
            ema_preds_2d = model_2d.inference(data_batch_trg)
            all_ema_preds_3d = model_3d(data_batch_trg)
        ...
        loss_trg_2d = F.cross_entropy(ori_logit_2d, data_batch_trg['seg_label'][pc2img_mask].long().detach(), weight=class_weights)
        loss_trg_3d = F.cross_entropy(preds_3d['seg_logit'], data_batch_trg['seg_label'].long().detach(), weight=class_weights)
        train_metric_logger.update(
            loss_trg_2d=loss_trg_2d,
            loss_trg_3d=loss_trg_3d
            )
        loss_2d.append(loss_trg_2d)
        loss_3d.append(loss_trg_3d)
