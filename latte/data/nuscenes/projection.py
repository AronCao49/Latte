...
def map_pointcloud_to_image(pc, im_shape, info, im=None):
    ...
    Tr_curr = np.eye(4)
    Tr_curr[:3, :3] = Quaternion(info['lidar2ego_rotation']).rotation_matrix
    Tr_curr[:3, 3] = np.array(info['lidar2ego_translation'])
    Tr_p2c = Tr_curr @ Tr_p2c
    ...
    Tr_curr = np.eye(4)
    Tr_curr[:3, :3] = Quaternion(info['ego2global_rotation_lidar']).rotation_matrix
    Tr_curr[:3, 3] = np.array(info['ego2global_translation_lidar'])
    Tr_p2c = Tr_curr @ Tr_p2c
    ...
    Tr_curr = np.eye(4)
    Tr_curr[:3, :3] = Quaternion(info['ego2global_rotation_cam']).rotation_matrix
    Tr_curr[:3, 3] = np.array(info['ego2global_translation_cam'])
    Tr_curr = np.linalg.inv(Tr_curr)
    Tr_p2c = Tr_curr @ Tr_p2c
    ...
    Tr_curr = np.eye(4)
    Tr_curr[:3, :3] = Quaternion(info['cam2ego_rotation']).rotation_matrix
    Tr_curr[:3, 3] = np.array(info['cam2ego_translation'])
    Tr_curr = np.linalg.inv(Tr_curr)
    Tr_p2c = Tr_curr @ Tr_p2c
    ...
    view = np.array(info['cam_intrinsic'])
    viewpad = np.eye(4)
    viewpad[:view.shape[0], :view.shape[1]] = view
    Tr_p2c = viewpad @ Tr_p2c
    ...
    return mask, pc.T, points.T[:, :2], Tr_p2c